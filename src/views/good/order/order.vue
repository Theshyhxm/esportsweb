...
<html class="no-js" lang="zxx">
 ...
 <body>
  ...
  <section class="vs-match-wrapper vs-match-layout1 newsletter-pb">
   <div class="container">
    <div class="mb-15 filter-active row" id="Myorder">
       ...
       <div class="row align-items-center justify-content-center justify-content-lg-start">
        ...
        <div class="col-lg">
         <div class="match-about pl-35">
          <h3 class="team-name h5 text-white mb-1 mb-lg-2"><a href="cartController/findOrderItem?orderId=${order.orderId}">order number:${order.orderId}</a></h3>
          <p class="match-date mb-0">Date:${order.createTime}</p>
         </div>
        </div>
...
<script>
 function find1(pageNo){
  $.ajax({
   type: "post",
   url: "cartController/findOrder",
   data:"uid=${sessionScope.user.uid}&pageNo="+pageNo,//json对象转化为json字符串
   success: function (orders) {
    console.log(orders)
    $("#Myorder").empty();
    $("#Myorder").append(
            "<input type='hidden' id='prepageNo' value='"+orders.prePage+"' '>"+
            "<input type='hidden' id='nextPage' value='"+orders.nextPage+"' '>"+
            "<input type='hidden' id='pageNum' value='" + orders.pageNum + "' '>"+
            "<input type='hidden' id='endPage' value='" + orders.pages + "' '>"
    )
   ...
   "<div class='col-md-6 col-lg-12 grid-item fortnite csgo dota'>" +
             "<div class='vs-match'>" +
             "<div class='row align-items-center justify-content-center justify-content-lg-start'>" +
             "<div class='col-auto col-xl-2 text-end' id='status"+i+"'>"+
      "</div><div class='col-lg'><div class='match-about pl-35'>"+
         " <h3 class='team-name h5 text-white mb-1 mb-lg-2'><a href='cartController/findOrderItem?orderId="+orders.list[i].orderId+"'>order number:"+orders.list[i].orderId+"</a></h3>"+
          "<p class='match-date mb-0'>Date:"+orders.list[i].createTime+"</p>"+
             "</div></div><div class='col-auto col-xl-2'><div class='match-time text-white'>"+
   " Price:"+orders.list[i].price+
         "</div></div><div class='col-auto col-xl-2 text-end'></div></div></div></div>"
     )
     if(orders.list[i].status==0){
      $("#status"+i).append(
              "未发货"
      )
     }
